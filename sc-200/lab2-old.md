...
> **Important**
>
> This lab uses the live Microsoft Defender XDR tenant provisioned for this course, not the original Microsoft interactive click-through simulations (those are no longer available). Because no incident exists yet in a freshly provisioned tenant, **Part 0** below has you generate a real, live incident first. Parts 1 and 2 then investigate that incident directly in the Defender XDR portal.
...
## Part 0: Generate an incident to investigate

> **Note:** Devices and user accounts in this trial tenant are limited (typically a single **WIN1** device and the **Admin** account), so "affected device" and "affected user" throughout this lab will refer to that device/account rather than a wider fleet. That's expected in this environment.
...
3. Sign in with the tenant admin account and password provided by CloudLabs.

4. In the left-hand menu, under **Assets**, select **Devices**. Wait until **WIN1** appears in the Devices list before continuing.

    > **Note:** If WIN1 doesn't appear after about an hour, it indicates an onboarding or connectivity problem — check with your instructor before continuing.

5. In the left menu, expand **System** and select **Settings**, then select **Endpoints**.

6. Select **Onboarding** under Device management and confirm **"Windows 10 and 11"** is selected as the operating system. The **"First device onboarded"** message should show **Completed**.

7. Scroll down to section **"2. Run a detection test"** and select **Copy** to copy the detection test script.

8. On the WIN1 virtual machine, search for **CMD**, right-click **Command Prompt**, and choose **Run as Administrator**. Select **Yes** on the User Account Control prompt.

9. Right-click inside the Administrator: Command Prompt window to paste the script, then press **Enter** to run it.

    > **Note:** The window closes automatically once the script runs successfully. A basic test alert appears in the portal within a few minutes — this only confirms detection is working; the richer incident used for the rest of this lab comes from Task 2 below.

### Task 2: Simulate an attack to generate a real incident

> **Warning:** Only run this simulated attack in the course-provided CloudLabs tenant.
...
2. Navigate to the **\Users\Admin\Desktop\Allfiles** folder, type `.\AttackScript.ps1`, and press **Enter**. When prompted, type **R** and press **Enter** to *Run Once*.

3. The script produces several lines of output and a message that it *Failed to resolve Domain Controllers in the domain*. Shortly after, Notepad opens automatically with simulated attack code injected into it, which attempts to communicate with an external IP address (simulating a command-and-control server). Leave this Notepad window open.

4. In the Microsoft Defender XDR portal, expand **Investigation & response**, expand **Incidents & alerts**, and select **Incidents**.

    > **Note:** On newer versions of the portal, *Incidents & alerts* is found under *Investigations & response*. If the incident doesn't appear yet, wait a few minutes and refresh, or clear the *Alert severity* filter.

5. Confirm a new incident named **"Multi-stage incident involving Defense evasion & Discovery on one endpoint"** appears. This is the incident you'll investigate for the rest of this lab.
...
3. Open **"Multi-stage incident involving Defense evasion & Discovery on one endpoint"** — the incident you generated in Part 0.
